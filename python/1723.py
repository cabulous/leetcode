...
class Solution:
    ...
    def minimumTimeRequired(self, jobs: List[int], k: int) -> int:
        ...
        while left <= right:
            ...
            if self.can_reach(0, [mid] * self.k, mid):
                right = mid - 1
            else:
                left = mid + 1
    ...
    def can_reach(self, curr_job_index, time, origin_val):
        ...
        for i in range(self.k):
            if time[i] >= self.jobs[curr_job_index]:
                ...
                if self.can_reach(curr_job_index + 1, time, origin_val):
                    return True
                time[i] += self.jobs[curr_job_index]
            if time[i] == origin_val:
                return False
